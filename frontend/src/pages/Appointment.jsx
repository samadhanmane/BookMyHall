...
const Appointment = () => {
  ...
  const getAvailableSlots = () => {
    ...
    // Different time slots based on facility type
    let timeSlots = [];
    if (HallInfo.isVehicle) {
      timeSlots = [
        { time: "Full Day (8:00 AM – 10:00 PM)", datetime: new Date(selectedDay.setHours(8, 0)) }
      ];
    } else if (HallInfo.isGuestRoom) {
      timeSlots = [
        { time: "Full Day (10:00 AM – 5:00 PM)", datetime: new Date(selectedDay.setHours(10, 0)) }
      ];
    } else {
      timeSlots = [
        { time: "Morning (10:00 AM – 1:00 PM)", datetime: new Date(selectedDay.setHours(10, 0)) },
        { time: "Afternoon (2:00 PM – 5:00 PM)", datetime: new Date(selectedDay.setHours(14, 0)) },
        { time: "Full Day (10:00 AM – 5:00 PM)", datetime: new Date(selectedDay.setHours(10, 0)) },
      ];
    }
