...
const SpecialityMenu = () => {
  return (
    <div className="flex flex-col items-center gap-4 py-16 text-[#030303] font-poppins bg-white" id="speciality">
      <h1 className="text-3xl font-semibold">Browse Facilities by Type</h1>
      <p className="sm:w-1/3 text-center text-sm text-[#555]">
        Select a facility type to get started. Book halls, guest rooms, or vehicles for your events and needs with ease and efficiency.
      </p>
        ...
        {specialityData.map((item, index) => (
          ...
          >
            <img
              src={item.image}
              alt={item.speciality}
              className="w-20 h-20 sm:w-24 sm:h-24 mb-2 rounded-full object-cover border-2 border-[#123458]"
            />
            <p>{item.speciality}</p>
          </Link>
        ))}
